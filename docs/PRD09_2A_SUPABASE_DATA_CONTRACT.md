...
**Brecha de migraciones existentes:** `supabase/legacy_migrations/202609180001_prd02_base.sql` y posteriores son artefactos legacy, no la implementación de este contrato. Definen, entre otras cosas, `products` por `organization_id` con un índice sobre descripción y `public.users` con roles anteriores; eso contradice la identidad por cadena/producto y `auth.users → profiles` aquí fijadas. 09.2B verificó que no estaban aplicadas en el proyecto nuevo, las archivó sin alterar su contenido y preparó un baseline nuevo; su aplicación remota aún requiere aprobación explícita. `SupabasePersistenceProvider` y `SupabaseDataProvider` reales se implementarán y probarán en 09.2G, no en este PRD.
