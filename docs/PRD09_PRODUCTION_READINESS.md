...
| --- | --- | --- |
| Code Complete | **NO** | Componentes locales y CI están implementados, pero faltan build Docker verificado y E2E web autenticado en hosting. |
| Hosting Ready | **NO** | FastAPI standalone pasó; Dockerfile preparado, Docker no instalado en esta máquina y despliegue no ejecutado. |
...
| Deployment Ready | **NO** | Requiere CI verde, prueba de contenedor/volumen, identidad del hosting y rollback ensayado en staging. |
...
- 43 pruebas del assistant/backend y 96 de motores pasaron localmente (139 en total). Cobertura del paquete operativo `assistant_api`: **81%**, registrada como piso de CI; se excluyeron tests y entradas CLI del denominador. Lint, type-check y build frontend pasaron. CI remota aún debe confirmarlo.
...
| 07 | PASS local | Escáner de archivos rastreados; CI aún por ejecutar. |
| 08 | PENDIENTE | Docker build en CI; no hay Docker local. |
...
| 32–33 | PENDIENTE | Workflows CI definidos; su ejecución remota debe confirmarse. |
...
1. CI completa verde en GitHub, incluido Docker build y scanner en el repositorio público.
2. Staging con FastAPI hospedado, volumen SQLite persistente y prueba de backup/restore/rollback real.
3. Verificación del contrato de identidad del hosting: encabezados de usuario autenticado no falsificables desde navegador; allowlist y HTTPS reales.
4. E2E autenticado frontend→FastAPI y comparación visual antes/después en staging; baseline de carga del Dashboard con sesión válida.
5. Política operativa de reinicio de jobs pendientes y coordinación compartida si se usan varios workers/instancias. Mientras tanto, un solo worker.
