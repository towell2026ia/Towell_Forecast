...
`test` usa los mismos proveedores locales, con rutas temporales inyectadas en pruebas. `staging` y `production` exigen origen HTTPS, secreto HMAC y allowlist gerencial. `production` deshabilita `/docs`, `/redoc` y `/openapi.json`. No existe conmutación silenciosa a OpenAI o Supabase.

El frontend necesita `PYTHON_ASSISTANT_URL` y el mismo `ASSISTANT_API_TOKEN` sólo en el servidor, nunca en variables `NEXT_PUBLIC_*`. En un hosting ChatGPT Site, el adaptador de identidad lee el ID de usuario autenticado del lado servidor. Antes de habilitarlo públicamente hay que demostrar que el proxy del hosting elimina encabezados de identidad enviados por el navegador. Sin esa garantía, mantener `AI_ASSISTANT_API_ENABLED=false`.
